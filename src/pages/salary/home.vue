<template>
	<view>
		<view class="top-txt">
			...
			<view class="right">
				{{title}} · <text class="link-txt" @click="jumpRule" style="margin-right: 30rpx;" >考勤规则</text>
			</view>
		</view> 
		...
		<view class="clock-wrap" v-if="type">
			<view @click="doSalary" class="clock-btn" style="position: relative;">
				<image :src="bgurls[0].img" style="position: absolute;width: 100%;height: 100%;z-index: 5;"  v-if="type.class==1 || type.class==2"></image>
				<image :src="bgurls[2].img" style="position: absolute;width: 100%;height: 100%;z-index: 5;"  v-else></image>
				<view class="txt" style="position: relative;z-index: 10;">{{type.name}}</view>
				<view class="time" style="position: relative;z-index: 10;">{{timeStr}}</view>
			</view>
		...
		<block v-if="type">
			<button class="out-btn" v-if="type.class==1 || type.class==2" @click="wqdkOpen">外勤打卡</button>
		</block>
		...
		<uni-popup ref="popup" type="bottom" :animation="true">
			<view class="create-wrap">
				<view class="tit">上班时间: <view style="width: 500rpx;">{{s_time}}</view>
				</view>
				 <!-- @click="salaryInfo" -->
				<view class="tit">打卡地点: <view style="width: 500rpx;">{{address}}<view  @click="chongxdw"  class="re-ads">重新定位</view></view>
				</view>
				<view class="tit">添加图片:
					<view class="item-box">
						...
						<image @click="choseimg" mode="aspectFill" src="/static/image/morephoto@3x.png"></image>
					</view>
				</view>
		...
		methods: {
			chongxdw(){
				uni.getLocation({
					...
					success: function(res) {
						...
						_this.address = res.address.district +res.address.street+res.address.poiName
			...
			salaryInfo() {
				common.request('Salary_Fans/salaryInfo', {}, function(res) {
					...
					_this.GetDistance(res.data.info.lat, res.data.info.lng, res.data.info.scope);
				});
			},
			doSalary() {
				if (this.isInSalary) {
			...
			egress(){
				common.request('Salary_Fans/egress', {
					address: this.address,
					note: this.note,
