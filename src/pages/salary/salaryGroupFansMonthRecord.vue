	...
	<view>
		<view id="calendar">
			...
			<view class="toggle-type">
				<navigator open-type="redirect" :url="'salaryGroupFansRecord?uid='+uid">
...
<script>
	...
	export default {
		...
		methods: {
			...
			getTotal(month = 0) {
				common.request('Salary_Group_Fans_Log/monthinfo', {
					month: month,
					uid: this.uid,
				}, function(res) {
					if (res.data.status == 1) {
			...
			getStatus(status) {
				common.request('Salary_Group_Fans_Log/getStatus', {
					status: status,
					month: _this.month,
					uid: this.uid,
				}, function(res) {
