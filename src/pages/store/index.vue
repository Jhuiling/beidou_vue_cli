	...
	<view>
		...
		<div class="nav-bar-two" style="z-index:1000;position: fixed;bottom:0;height:120rpx;">
			...
			<div class="nav-bar-item" @click="navTo('/pages/store/index')">
				<image class="nav-bar-item-icon" src="../../static/image/s_store@3x.png"></image>
	...
	export default {
		...
		methods: {
			disanf(e){
				...
				// }  
				// uni.navigateTo({
				// 	url: '/pages/index_two/index'
				// })
			},
			getlist() {
				// if (!loadMore) return false;
				// loadMore = false
				common.request('goods/listdata/page/' + this.page, {}, function(res) {
			...
			getapp() {
				common.request('Industry/listdata', {}, function(res) {
					uni.stopPullDownRefresh();
					if (res.data.status != 1) return false;
