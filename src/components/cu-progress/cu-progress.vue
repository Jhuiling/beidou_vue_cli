<template>
		<view class="cu-progress" :style="{'border-radius':bgBR,'width': cuProgressW,'height':cuProgressH,'background-color': backgroundColor}">
			...
			<movable-area friction="100" class="area" :style="{width:areaW,height:areaH}" @touchcancel="touchCancel" @touchstart="areaTouchStart" @touchmove="areaTouchMove" @touchend="touchEnd">
				<movable-view  :animation="false" :style="{'top': handleT,'width':handleS,'height':handleS,'border-radius': handleBR,'background-color':handleColor}"  @change="change" :damping="damping" :x="handleX" :y="handleY" direction="horizontal">
				 <view class="handle"></view>
				 <view class="current">{{currentNum}}</view>
				 </movable-view>
			</movable-area>
		</view>
</template>
